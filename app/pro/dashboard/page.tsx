...
export default async function DashboardPage() {
  ...
  const [{ count: totalPatients }, { data: recentPatients }] = await Promise.all([
    supabase.from('patients').select('*', { count: 'exact', head: true }).eq('professional_id', PROF_ID).eq('active', true),
    supabase.from('patients').select('id,full_name,goal,weight_kg,created_at').eq('professional_id', PROF_ID).eq('active', true).order('created_at', { ascending: false }).limit(5),
  ])
    ...
    <div>
      ...
      <div className="p-8">
        ...
        <div className="grid grid-cols-3 gap-4 mb-8">
          <Link href="/pro/pacientes" className="card p-5 hover:border-pgf-200 hover:shadow-md transition-all group">
          ...
          <Link href="/pro/alimentos" className="card p-5 hover:border-pgf-200 hover:shadow-md transition-all group">
            <div className="text-3xl mb-2">🥦</div>
          ...
          <Link href="/pro/exercicios" className="card p-5 hover:border-pgf-200 hover:shadow-md transition-all group">
            <div className="text-3xl mb-2">🎥</div>
          ...
          <table className="w-full">
            <thead>
              <tr className="text-left">
                <th className="px-5 py-3 text-[11px] font-semibold text-gray-400 uppercase tracking-wide bg-gray-50 border-b">Paciente</th>
                <th className="px-5 py-3 text-[11px] font-semibold text-gray-400 uppercase tracking-wide bg-gray-50 border-b">Objetivo</th>
                <th className="px-5 py-3 text-[11px] font-semibold text-gray-400 uppercase tracking-wide bg-gray-50 border-b">Peso</th>
                <th className="px-5 py-3 text-[11px] font-semibold text-gray-400 uppercase tracking-wide bg-gray-50 border-b"></th>
              </tr>
            </thead>
            <tbody>
