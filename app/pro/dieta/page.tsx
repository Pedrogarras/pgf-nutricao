...
export default async function DietaListPage() {
  ...
  return (
    <div>
      ...
      >
        <div>
          <h1 className="text-base font-bold text-white">Prescrição Dietética</h1>
          <p className="text-xs" style={{ color: 'rgba(255,255,255,0.35)' }}>Selecione um paciente</p>
        </div>
      </div>
      <div className="p-8">
        <div className="grid grid-cols-3 gap-4">
          {(patients ?? []).map(p => (
            <Link key={p.id} href={`/pro/pacientes/${p.id}`} className="card p-5 hover:border-pgf-200 hover:shadow-md transition-all group">
              ...
              <div className="text-xs text-gray-400">{p.goal ?? 'Sem objetivo definido'}</div>
              {p.weight_kg && <div className="text-xs text-gray-400">{p.weight_kg} kg</div>}
            </Link>
          ))}
          {!patients?.length && (
