  ...
  return (
    <div>
      <div className="sticky top-0 z-40 bg-white border-b border-gray-100 px-8 h-15 flex items-center justify-between">
        <div><h1 className="text-lg font-bold">Prescrição de Dieta</h1><p className="text-xs text-gray-400">Selecione um paciente</p></div>
      </div>
      <div className="p-8">
        <div className="grid grid-cols-3 gap-4">
          {(patients ?? []).map(p => (
            <Link key={p.id} href={`/pro/pacientes/${p.id}/dieta`} className="card p-5 hover:border-pgf-200 hover:shadow-md transition-all group">
